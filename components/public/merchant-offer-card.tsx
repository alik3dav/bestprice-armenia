...
export function MerchantOfferCard({
  ...
  return (
    <article
      className={`group relative overflow-hidden rounded-2xl border bg-white px-3 py-3 shadow-sm transition duration-200 hover:-translate-y-0.5 hover:shadow-md sm:px-4 ${
        isBestOffer
          ? "border-[var(--color-brand-red)] ring-1 ring-red-100"
          : "border-[var(--color-border-muted)] hover:border-slate-300"
      }`}
    >
      ...
      <div className="grid gap-3 md:grid-cols-[48px_minmax(0,1fr)_minmax(130px,170px)] md:items-center lg:grid-cols-[48px_minmax(0,1fr)_minmax(130px,160px)_132px]">
        <div className="flex items-center gap-3 md:block">
          <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl border border-[var(--color-border-muted)] bg-slate-50 p-2">
        ...
        <div className="min-w-0 space-y-2">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {isBestOffer ? <span className="inline-flex items-center gap-1 rounded-full bg-red-50 px-2 py-1 text-[11px] font-semibold text-[var(--color-brand-red)] ring-1 ring-red-100"><ShieldCheck className="h-3.5 w-3.5" /> Լավագույն</span> : null}
          </div>
        ...
        <div className="rounded-xl bg-slate-50 px-3 py-2 md:justify-self-end md:bg-transparent md:px-0 md:py-0 md:text-right">
        ...
        <div className="md:col-span-3 lg:col-span-1 lg:justify-self-end">
          {productUrl ? (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-[var(--color-brand-red)] px-3 py-2 text-sm font-semibold text-white transition hover:bg-[var(--color-brand-red-hover)] focus:outline-none focus:ring-2 focus:ring-red-200 lg:w-[132px]"
            >
              Տեսնել <ArrowUpRight className="h-4 w-4" />
            </a>
          ) : (
            <div className="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-xl border border-dashed border-slate-300 px-3 py-2 text-sm font-semibold text-slate-500 lg:w-[132px]"><Store className="h-4 w-4" /> Հղում չկա</div>
          )}
        </div>
      </div>
    </article>
