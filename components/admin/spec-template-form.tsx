...
export function SpecTemplateForm({ categories, action, backHref, submitLabel, submitLoadingLabel, defaultValues }: { categories: Category[]; action: (formData: FormData) => void; backHref: Route; submitLabel: string; submitLoadingLabel: string; defaultValues?: { name: string; categoryId: string; groups: SpecTemplateGroupDraft[]; fields: SpecTemplateFieldDraft[] } }) {
  ...
  const payload = useMemo(() => JSON.stringify({
    ...
    fields: fields.map((f, i) => ({ name: f.name.trim(), key: f.key.trim(), fieldType: f.fieldType, required: f.required, sortOrder: f.sortOrder ?? i, options: f.optionsText.split(/\r?\n|,/).map((o) => o.trim()).filter(Boolean), groupId: f.groupId }))
  }), [groups, fields]);
        ...
        {groups.map((group, gIndex) => {
          ...
          return <div key={group.id} className="rounded border border-slate-200">
            <div className="flex flex-wrap items-center gap-2 border-b bg-slate-50 px-2 py-1.5">
              ...
              <button type="button" className="rounded border px-2 py-1 text-xs" onClick={() => setFields((prev) => [...prev, emptyField(group.id, gFields.length)])}>Add field</button>
            ...
            <div className="hidden overflow-x-auto md:block">
              <table className="min-w-full text-xs"><thead><tr className="border-b bg-slate-50 text-left"><th className="px-2 py-1">Group</th><th className="px-2 py-1">Label/Name</th><th className="px-2 py-1">Key/Slug</th><th className="px-2 py-1">Type</th><th className="px-2 py-1">Required</th><th className="px-2 py-1">Display Order</th><th className="px-2 py-1">Options</th><th className="px-2 py-1">Actions</th></tr></thead><tbody>
                {gFields.map((field) => <tr key={field.id} className="border-b last:border-b-0 align-top"><td className="px-2 py-1.5">{group.name || "—"}</td><td className="px-2 py-1.5"><input required value={field.name} onChange={(e) => setFields((p) => p.map((f) => f.id === field.id ? { ...f, name: e.target.value } : f))} className="w-36 rounded border px-2 py-1" /></td><td className="px-2 py-1.5"><input required value={field.key} onChange={(e) => setFields((p) => p.map((f) => f.id === field.id ? { ...f, key: e.target.value } : f))} className="w-36 rounded border px-2 py-1" /></td><td className="px-2 py-1.5"><select value={field.fieldType} onChange={(e) => setFields((p) => p.map((f) => f.id === field.id ? { ...f, fieldType: e.target.value as FieldType } : f))} className="rounded border px-2 py-1"><option value="text">Text</option><option value="number">Number</option><option value="boolean">Boolean</option><option value="select">Select</option><option value="multi-select">Multi-select</option></select></td><td className="px-2 py-1.5"><input type="checkbox" checked={field.required} onChange={(e) => setFields((p) => p.map((f) => f.id === field.id ? { ...f, required: e.target.checked } : f))} /></td><td className="px-2 py-1.5"><input type="number" value={field.sortOrder} onChange={(e) => setFields((p) => p.map((f) => f.id === field.id ? { ...f, sortOrder: Number(e.target.value) } : f))} className="w-16 rounded border px-2 py-1" /></td><td className="px-2 py-1.5">{field.fieldType === "select" || field.fieldType === "multi-select" ? <input value={field.optionsText} onChange={(e) => setFields((p) => p.map((f) => f.id === field.id ? { ...f, optionsText: e.target.value } : f))} className="w-40 rounded border px-2 py-1" placeholder="A,B,C" /> : <span className="text-slate-400">—</span>}</td><td className="px-2 py-1.5"><button type="button" className="rounded border px-2 py-1" onClick={() => setFields((p) => p.filter((f) => f.id !== field.id))}>Remove</button></td></tr>)}
              </tbody></table>
            </div>

            <div className="space-y-2 p-2 md:hidden">{gFields.map((field) => <div key={field.id} className="rounded border p-2 text-xs"><div className="grid grid-cols-2 gap-2"><input value={field.name} onChange={(e) => setFields((p) => p.map((f) => f.id === field.id ? { ...f, name: e.target.value } : f))} className="rounded border px-2 py-1" placeholder="Label" /><input value={field.key} onChange={(e) => setFields((p) => p.map((f) => f.id === field.id ? { ...f, key: e.target.value } : f))} className="rounded border px-2 py-1" placeholder="key_slug" /><select value={field.fieldType} onChange={(e) => setFields((p) => p.map((f) => f.id === field.id ? { ...f, fieldType: e.target.value as FieldType } : f))} className="rounded border px-2 py-1"><option value="text">Text</option><option value="number">Number</option><option value="boolean">Boolean</option><option value="select">Select</option><option value="multi-select">Multi</option></select><input type="number" value={field.sortOrder} onChange={(e) => setFields((p) => p.map((f) => f.id === field.id ? { ...f, sortOrder: Number(e.target.value) } : f))} className="rounded border px-2 py-1" placeholder="Order" /></div><div className="mt-2 flex items-center justify-between"><label className="flex items-center gap-1"><input type="checkbox" checked={field.required} onChange={(e) => setFields((p) => p.map((f) => f.id === field.id ? { ...f, required: e.target.checked } : f))} />Required</label><button type="button" className="rounded border px-2 py-1" onClick={() => setFields((p) => p.filter((f) => f.id !== field.id))}>Remove</button></div></div>)}</div>
          </div>;
        })}
      </div>
    </div>
