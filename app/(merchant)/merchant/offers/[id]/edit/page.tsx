...
export default async function EditOfferPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams?: Promise<{ error?: string }> }) {
  ...
  async function updateOffer(formData: FormData) {"use server";
    ...
    const { error } = await c.from("product_offers").update({ product_id: d.productId, price: d.price, currency: d.currency, stock_status: d.stockStatus, status: d.status, merchant_sku: d.merchantSku || null, product_url: d.productUrl || null, notes: d.notes || null }).eq("id", id).eq("merchant_id", linkedMerchant.id);
