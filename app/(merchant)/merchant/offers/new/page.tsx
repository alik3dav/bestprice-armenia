...
const schema = z.object({ productId: z.string().uuid(), price: z.coerce.number().nonnegative(), currency: z.string().trim().min(1), stockStatus: z.enum(["in_stock","limited","out_of_stock","preorder"]), status: z.enum(["draft","active","archived"]), merchantSku: z.string().optional(), productUrl: z.string().url().optional().or(z.literal("")), notes: z.string().optional() });
...
export default async function NewOfferPage({ searchParams }: { searchParams?: Promise<{ error?: string; success?: string }> }) {
  ...
  async function createOffer(formData: FormData) {"use server";
    ...
    const { error } = await c.from("product_offers").insert({ merchant_id: linkedMerchant.id, product_id: d.productId, price: d.price, currency: d.currency, stock_status: d.stockStatus, status: d.status, product_url: d.productUrl || null, merchant_sku: d.merchantSku || null, notes: d.notes || null, created_by: authUser.id });
