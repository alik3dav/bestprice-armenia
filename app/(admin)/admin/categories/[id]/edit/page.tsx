...
const categorySchema = z.object({ name: z.string().trim().min(2), slug: z.string().trim().min(2).regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/), status: z.enum(["draft", "active", "archived"]), imageUrl: z.string().trim().optional() });
...
export default async function CategoryEditPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams?: Promise<{ error?: string }> }) {
  ...
  const { data: category } = await supabase.from("categories").select("id,name,slug,status,image_url").eq("id", id).single();
