...
export default async function ProductEditPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams?: Promise<{ error?: string }> }) {
  ...
  const [productResult, categoriesResult, brandsResult, templatesResult, templateGroupsResult, fieldsResult, valuesResult] = await Promise.all([
  ...
  return <section className="space-y-4 rounded border bg-white p-4"><div className="flex items-center justify-between gap-3"><div><h1 className="text-xl font-semibold">Edit Product</h1><p className="text-sm text-slate-500">Update product details in the catalog.</p></div><Link href="/admin/products" className="rounded border px-3 py-2 text-sm hover:bg-slate-50">Back to products</Link></div>{(query?.error || categoriesResult.error || brandsResult.error || fieldsResult.error || templateGroupsResult.error || templatesResult.error || valuesResult.error) ? <div className="rounded border border-red-200 bg-red-50 p-3 text-sm text-red-700">{query?.error ?? categoriesResult.error?.message ?? brandsResult.error?.message ?? fieldsResult.error?.message ?? templateGroupsResult.error?.message ?? templatesResult.error?.message ?? valuesResult.error?.message}</div> : null}<ProductForm action={updateProduct} categories={categoriesResult.data ?? []} brands={brandsResult.data ?? []} templatesByCategory={templatesByCategory} backHref="/admin/products" submitLabel="Save product" submitLoadingLabel="Saving..." defaultValues={{ title: p.title, slug: p.slug, categoryId: p.category_id, brandId: p.brand_id ?? "", model: p.model ?? "", description: p.description ?? "", status: p.status, images: Array.isArray(p.images) ? p.images.join("\n") : "", specValues }} /></section>;
}
