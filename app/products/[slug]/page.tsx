...
export default async function ProductDetailPage({ params }: PageProps) {
  ...
  return <main className="min-h-screen bg-white text-slate-900">{/* unchanged layout below */}
    ...
    <section className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8"><Link href="/" className="inline-block text-sm text-slate-600 hover:underline">← Back to products</Link>
      <article className="mt-4 grid gap-8 lg:grid-cols-2"><div className="space-y-3"><div className="relative aspect-square overflow-hidden rounded-2xl bg-[#f6f6f6] p-4">{imageList[0] ? <img src={imageList[0]} alt={product.title} className="h-full w-full object-contain" /> : <div className="flex h-full items-center justify-center text-sm text-slate-400">No image available.</div>}</div>{imageList.length > 1 ? <div className="grid grid-cols-4 gap-2 sm:grid-cols-6">{imageList.slice(1).map((image, idx) => <div key={`${image}-${idx}`} className="aspect-square overflow-hidden rounded-lg border bg-[#f6f6f6] p-1"><img src={image} alt={`${product.title} image ${idx + 2}`} className="h-full w-full object-contain" /></div>)}</div> : null}</div>
      <div className="space-y-4">{category ? <p className="text-xs font-medium uppercase tracking-wide text-slate-500">{category.name}</p> : null}<h1 className="text-3xl font-bold tracking-tight sm:text-4xl">{product.title}</h1><p className="text-sm leading-6 text-slate-600">{product.short_description || "No short description available."}</p><div className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><p className="text-xs uppercase tracking-wide text-slate-500">Lowest active offer</p>{lowestOffer ? <p className="mt-1 text-3xl font-bold">{lowestOffer.price} {lowestOffer.currency}</p> : <p className="mt-1 text-sm text-slate-500">No active offers available right now.</p>}</div></div></article>
